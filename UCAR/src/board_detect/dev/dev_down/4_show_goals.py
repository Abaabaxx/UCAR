...
# 5. 在计算每个目标位姿后立即记录详细数值到日志
# 6. 筛选掉同时满足位置接近(2.0, 4.75)且姿态接近0度的位姿
...
class ClusterVisualizer:
    def __init__(self):
        ...
        # 发布者 - 用于发布目标位姿数组
        self.target_poses_pub = rospy.Publisher('/target_poses_array', PoseArray, queue_size=10)
        ...
        rospy.loginfo("ROI Polygon defined for filtering: [(-0.2, 3.5), (2.7, 3.5), (2.7, 5.2), (-0.2, 5.2)]")
        rospy.loginfo("Target poses will be generated %.2fm in front of each cluster", self.target_distance)
        rospy.loginfo("Filtering out poses near (%.1f, %.2f) Yaw: %.1f deg",
                    self.filter_target_x, self.filter_target_y, np.degrees(self.filter_target_yaw))
        rospy.loginfo("Position threshold: %.2f m, Angle threshold: %.1f deg (%.3f rad)",
                    self.filter_pos_threshold, self.filter_angle_threshold_deg, self.filter_angle_threshold_rad)
        
        # 在初始化后立即发布ROI多边形用于可视化
    ...
    def publish_roi_polygon(self):
        ...
        rospy.logdebug("Published ROI polygon visualization")
    ...
    def scan_callback(self, scan):
        ...
        for i, cluster in enumerate(final_filtered_clusters):
            ...
            if len(cluster) >= 2:
                ...
                # 确保方向向量不为零
                if norm < 1e-6:
                    rospy.logdebug("Skipping cluster with zero direction vector")
                    continue
                ...
                if np.dot(normal_vector, lidar_to_center) < 0:
                    ...
                    rospy.logdebug("Normal vector flipped to point outward from lidar")
                ...
                # 打印目标位姿的详细信息到控制台
                rospy.loginfo("---------- Cluster #%d Target Pose ----------", i+1)
                rospy.loginfo("Position (x, y): (%.3f, %.3f)", target_position[0], target_position[1])
                rospy.loginfo("Orientation (Yaw): %.3f rad (%.1f deg)", target_yaw, np.degrees(target_yaw))
                rospy.loginfo("Orientation (Quaternion x,y,z,w): (%.3f, %.3f, %.3f, %.3f)",
        ...
        for pose_data in calculated_poses:
            ...
            if not filter_this_pose:
                ...
                rospy.loginfo("FILTERED OUT pose near target template: Pos (%.3f, %.3f), Yaw %.3f rad (%.1f deg)",
                             current_pos[0], current_pos[1], current_yaw, np.degrees(current_yaw))
                rospy.loginfo("  Distance to filter target: %.3fm (threshold: %.2fm)", 
                             distance, self.filter_pos_threshold)
                rospy.loginfo("  Angle diff to filter target: %.3f rad (%.1f deg) (threshold: %.1f deg)",
                             angle_diff, np.degrees(angle_diff), self.filter_angle_threshold_deg)
        
        # 构建并发布PoseArray消息，使用筛选后的final_valid_poses
        pose_array_msg = PoseArray()
        pose_array_msg.header.stamp = rospy.Time.now()
        pose_array_msg.header.frame_id = "map"
        
        for pose_data in final_valid_poses:
            pose = Pose()
            # 设置位置
        ...
        # 发布位姿数组
        self.target_poses_pub.publish(pose_array_msg)
        
        # 定期重新发布ROI多边形可视化（确保它始终可见）
        if rospy.Time.now().to_sec() % 5 < 0.1:  # 大约每5秒更新一次
            self.publish_roi_polygon()
        
        rospy.loginfo("Published %d filtered clusters, %d calculated poses, %d final valid poses", 
                     len(marker_array.markers), len(calculated_poses), len(final_valid_poses))
        if len(calculated_poses) > len(final_valid_poses):
            rospy.loginfo("Removed %d poses that were too close to the filter target", 
                         len(calculated_poses) - len(final_valid_poses))
