...
更新说明：
- 添加了日志发布频率控制，设置为1Hz (1秒一次)
- 使用定时器替代直接发布
- 缓存最新的识别结果

作者：abaabaxxx
创建时间：2025-05-01 14:26:04
"""
...
class QRCodeDetector:
    def __init__(self):
        ...
        # 创建1Hz的定时器用于发布信息
        rospy.Timer(rospy.Duration(1.0), self.publish_info)
    ...
    def publish_info(self, event):
        if self.latest_qr_text:  # 只有在有识别结果时才发布
            # 发布最新的二维码内容
            ...
            rospy.loginfo('任务类型设置为: %s (原始内容: %s)', 
