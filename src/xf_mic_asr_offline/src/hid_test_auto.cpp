...
int main(int argc, char *argv[])
{
	...
	ndHandle.param("appid", appid, std::string("3d974b8e"));//appid，需要更换为自己的
	...
	while (!is_boot)
	{
		...
		if ((double)(clock() - start_time) / CLOCKS_PER_SEC > 2.0) // 2秒超时
		{
			printf(">>>>>开机初始化超时，继续执行...\n");
			is_boot = 1;
			break;
		}
		ros::Duration(0.01).sleep(); // 减少休眠时间
	}
	printf(">>>>>开机成功！\n");

	//************* 新增设置唤醒词为“小飞小飞” *********************//
	std::string new_awake_word = "小飞小飞";
	Set_request_awake_word = false;
	int ret = set_awake_word(const_cast<char *>(new_awake_word.c_str()));
	if (ret == -3)
	{
		printf(">>>>>设置唤醒词失败：麦克风未打开\n");
	}
	else 
	{
		// 等待设置唤醒词的响应
		...
		if (Set_request_awake_word)
		{
			printf(">>>>>唤醒词已成功设置为：%s\n", new_awake_word.c_str());
		}
	}
	//**********************************************************//
