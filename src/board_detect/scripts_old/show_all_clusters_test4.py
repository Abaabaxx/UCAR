...
class ClusterVisualizer:
    def __init__(self):
        ...
        # 新增：最小聚类长度，只有达到这个数量时才开始进行基于整体方向的角度检查
        self.min_cluster_points_for_angle_check = 3
    ...
    def scan_callback(self, scan):
        ...
        # 使用新的基于端点方向向量的聚类方法
        clusters = self.cluster_by_distance_and_overall_direction(points)
    ...
    def cluster_by_distance_and_overall_direction(self, points, proximity_threshold=0.05):
        ...
        for i, point in enumerate(points):
            ...
            if len(current_cluster) == 0:
                ...
                # 检查是否达到进行角度检查的最小聚类长度
                if len(current_cluster) < self.min_cluster_points_for_angle_check:
                    # 聚类还太短，直接加入当前点
                    current_cluster.append(point)
                    continue
    ...
    # 之前的基于三点夹角的聚类函数，保留用于参考
    def cluster_by_distance_and_angle(self, points, proximity_threshold=0.05):
        clusters = []
        current_cluster = []
        ...
        for i, point in enumerate(points):
            ...
            if len(current_cluster) == 0:
                ...
                # 当前聚类至少有两个点，检查距离和角度
                prev_point_np = np.array(current_cluster[-1])
                prev_prev_point_np = np.array(current_cluster[-2])
                ...
                # 计算向量
                vector1 = prev_point_np - prev_prev_point_np
                vector2 = point_np - prev_point_np
                ...
                if angle_change <= self.max_direction_change_rad:
                    # 角度变化在容忍范围内，加入当前聚类
                    current_cluster.append(point)
                else:
                    # 角度变化太大，开始新的聚类
