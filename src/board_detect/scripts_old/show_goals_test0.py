...
class ClusterVisualizer:
    ...
    def scan_callback(self, scan):
        ...
        for i, cluster in enumerate(final_filtered_clusters):
            ...
            if len(cluster) >= 2:
                ...
                if np.linalg.norm(direction_vector) > 1e-6:
                    ...
                    # 3. 计算法线向量（垂直于方向向量）
                    # 这里使用2D旋转矩阵，旋转90度得到法线
                    normal_vector = np.array([-unit_direction[1], unit_direction[0]])
                    
                    # 确保法线指向远离激光雷达的方向（即聚类的"外侧"）
                    # 计算从激光雷达到聚类中心的向量
                    ...
                    # 4. 计算目标位置（中心点沿法线方向前进target_distance）
                    target_position = center_point + self.target_distance * normal_vector
                    
                    # 5. 计算目标偏航角（面向线段，即法线的反方向）
                    target_yaw = np.arctan2(-normal_vector[1], -normal_vector[0])
                    ...
                    rospy.logdebug("Generated target pose at (%.2f, %.2f) with yaw %.2f rad",
        ...
        rospy.loginfo("Published %d final clusters and %d target poses", 
                     len(marker_array.markers), len(pose_array_msg.poses))
